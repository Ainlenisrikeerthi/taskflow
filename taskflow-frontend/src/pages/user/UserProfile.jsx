...
import { Mail, ShieldCheck, CalendarDays, UserRound, RefreshCw, CheckCircle2, Activity, Pencil, Save, X } from "lucide-react";
...
export default function UserProfile() {
  ...
  async function load() {
    ...
    setError("");
    try {
      const [p, h] = await Promise.all([api.users.getMe(), api.assignments.getMyAll()]);
      setProfile(p);
      setName(p?.name || "");
      setHistory(h || []);
    } catch (e) {
  ...
  const active = history.filter((a) => a.isActive).length;
    ...
    <div>
      <div className="page-header">
        <div className="page-header-left">
          <div className="breadcrumbs"><span>User</span><span>/</span><span>Profile</span></div>
          <h1>My Profile</h1>
      ...
      {loading ? (
        ...
        <div className="profile-live-grid">
          <section className="card profile-identity">
            ...
            <h2>{profile.name}</h2>
            <span className="profile-role"><ShieldCheck size={15} />User</span>
            <p>Authenticated TaskFlow account</p>
            <div className="profile-mini-stats">
              <div><Activity size={17} /><strong>{active}</strong><small>Active</small></div>
              <div><CheckCircle2 size={17} /><strong>{completed}</strong><small>Completed</small></div>
            </div>
          </section>
